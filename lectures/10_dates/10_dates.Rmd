...
- You can use `parse_date()`, `parse_date_time()`, and `parse_time()` to 
  [parse](https://dcgerard.github.io/stat_412_612/lectures/06_tidyr/06_parsers.pdf) 
  a date/date-time/time from a string.

    ```{r}
    x <- parse_date("10/11/2020", "%m/%d/%Y")
    x
    class(x)
    
    y <- parse_date_time("10/11/2020 11:59:20", "%m/%d/%Y %H:%M:%S")
    y
    class(y)
...
- **Exercise**: Parse the following date-times.
    ...
    ## No dym_hms() function is defined, so need to use parse_date_time()
    parse_date_time("26 2004 05 UTC 11/11/11.444", "%d %Y %m %H/%M/%S")
    ```
...
- You can add durations to date-times, but you always add *seconds*, so if there
  is a leap day you get weird results (add a day but the time is not the same
...
- Adding a `period` takes into account leap days.
    ```{r}
    one_pm
    one_pm + days(1)
    ```
